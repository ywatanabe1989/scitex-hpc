...
Also wires module-import-time subprocess coverage: force-set
COVERAGE_PROCESS_START + COVERAGE_FILE so children spawned by
`subprocess.run([sys.executable, ...])` / `python -m` contribute
to coverage, and drop an idempotent `.pth` shim into site-packages
that calls `coverage.process_startup()` early. See
...
already sets COVERAGE_FILE to a tmp dir by the time conftest is loaded;
hence the force-set.
"""
...
def _ensure_subprocess_coverage_shim() -> None:
    ...
    pth = purelib / "_scitex_hpc_subprocess_coverage.pth"
    shim = (
        "import os, coverage\n"
